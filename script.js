...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 마우스 위치에 따른 맵 이미지 parallax 효과 (맵 화면)
    const mapContainer = document.querySelector('.map-container');
    const mapScreen = document.getElementById('map-screen');
    
    if (mapContainer && mapScreen) {
        let isMapScreenActive = mapScreen.classList.contains('active');
        
        // 맵 화면이 활성화될 때 이벤트 리스너 추가
        const addParallaxEffect = () => {
            const handleMouseMove = (e) => {
                if (!isMapScreenActive) return;
                ...
                // 최대 이동 거리 (픽셀 단위) - 마우스 방향과 반대로 이동
                const maxMove = 15;
                const moveX = -normalizedX * maxMove;
                const moveY = -normalizedY * maxMove;
                
                mapContainer.style.transform = `translate(${moveX}px, ${moveY}px)`;
            };
            ...
            // 문서 전체에서 마우스 이동 감지 (창 밖으로 나갔을 때도 감지)
            document.addEventListener('mousemove', handleMouseMove);
            mapScreen.addEventListener('mouseleave', handleMouseLeave);
            
            // 화면 전환 감지를 위한 MutationObserver
            const observer = new MutationObserver(() => {
                isMapScreenActive = mapScreen.classList.contains('active');
    ...
    if (document.getElementById('japan-stage')) {
        ...
        // 헤더 사각형 애니메이션 트리거 (약간의 지연 추가)
        setTimeout(() => {
            triggerHeaderSquareAnimation();
        }, 100);
    }
...
function updateMapTitleImages() {
    ...
    const japanTitleImg = mapScreen.querySelector('.japan-title');
    if (japanTitleImg && japanCompleted) {
        japanTitleImg.src = 'resource/title/TitleJp_F.png';
    }
    
    const chinaTitleImg = mapScreen.querySelector('.china-title');
    if (chinaTitleImg && chinaCompleted) {
        chinaTitleImg.src = 'resource/title/TitleCn_F.png';
    }
...
// 헤더 사각형 애니메이션 트리거 함수
function triggerHeaderSquareAnimation() {
    const headerSquare = document.querySelector('.header-square');
    if (headerSquare) {
        // 초기 상태로 리셋
        headerSquare.classList.remove('slide-in');
        headerSquare.style.transform = 'translateX(-100%)';
        
        // 브라우저 렌더링 후 애니메이션 시작
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                headerSquare.style.transform = '';
...
function showScreen(screenName) {
    ...
    if (screens[screenName]) {
        ...
        if (screenName === 'map') {
            ...
            const fork = mapScreen.querySelector('.title-fork-m-image');
            const knife = mapScreen.querySelector('.title-knife-m-image');
            const titleText = mapScreen.querySelector('.title-content h1');
            const headerSquare = mapScreen.querySelector('.header-square');
            const countryButtons = mapScreen.querySelectorAll('.map-country-btn');
            const countryTitles = mapScreen.querySelectorAll('.country-title-image');

            if (mapContainer) mapContainer.classList.remove('map-expand');
            ...
            if (fork) fork.classList.remove('map-hide');
            if (knife) knife.classList.remove('map-hide');
            if (titleText) {
                titleText.classList.remove('map-hide');
                titleText.classList.remove('map-shrink');
            }
            if (headerSquare) headerSquare.classList.remove('slide-in');
        ...
        if (screenName === 'japan' || screenName === 'china' || screenName === 'france') {
            const headerSquare = document.querySelector('.header-square');
            if (headerSquare) {
                headerSquare.classList.remove('slide-in');
                // 브라우저 렌더링 후 애니메이션 시작
...
// 타이틀 화면 Start 버튼 (TitleFr.png)
const startTitleBtn = document.getElementById('start-title-btn');
if (startTitleBtn) {
    startTitleBtn.addEventListener('click', () => {
        showScreen('map');
    });
}

// 타이틀 화면 Info 버튼 (TitledishFiinished.png)
const infoTitleBtn = document.getElementById('info-title-btn');
const infoModalOverlay = document.getElementById('info-modal-overlay');
const infoModalClose = document.getElementById('info-modal-close');

if (infoTitleBtn) {
    infoTitleBtn.addEventListener('click', () => {
        if (infoModalOverlay) {
            infoModalOverlay.style.display = 'flex';
            // 약간의 딜레이 후 fade-in 애니메이션
            setTimeout(() => {
...
if (japanMapBtn) {
    ...
    japanMapBtn.addEventListener('mouseenter', () => {
        if (japanTitle) {
            // 클리어 상태 확인 (이미지 src 또는 localStorage)
            const isCleared = japanTitle.src.includes('TitleJp_F.png') || 
                             localStorage.getItem('japanCompleted') === 'true' || 
                             japanCompleted;
            
            japanTitle.classList.add('title-hover');
            // 클리어되지 않은 경우에만 회전 시작
            if (!isCleared) {
                setTimeout(() => {
                    japanTitle.classList.add('title-rotate');
                }, 100);
            }
        }
    });
    japanMapBtn.addEventListener('mouseleave', () => {
        if (japanTitle) {
            japanTitle.classList.remove('title-hover', 'title-rotate');
        }
    });
...
if (chinaMapBtn) {
    const chinaTitle = document.querySelector('.china-title');
    chinaMapBtn.addEventListener('mouseenter', () => {
        if (chinaTitle) {
            // 클리어 상태 확인 (이미지 src 또는 localStorage)
            const isCleared = chinaTitle.src.includes('TitleCn_F.png') || 
                             localStorage.getItem('chinaCompleted') === 'true' || 
                             chinaCompleted;
            
            chinaTitle.classList.add('title-hover');
            // 클리어되지 않은 경우에만 회전 시작
            if (!isCleared) {
                setTimeout(() => {
                    chinaTitle.classList.add('title-rotate');
                }, 100);
            }
        }
    });
    chinaMapBtn.addEventListener('mouseleave', () => {
        if (chinaTitle) {
            chinaTitle.classList.remove('title-hover', 'title-rotate');
        }
    });
...
if (franceMapBtn) {
    const franceTitle = document.querySelector('.france-title');
    franceMapBtn.addEventListener('mouseenter', () => {
        if (franceTitle) {
            // 클리어 상태 확인 (이미지 src 또는 localStorage)
            const isCleared = franceTitle.src.includes('TitleFr_F.png') || 
                             localStorage.getItem('franceCompleted') === 'true' || 
                             franceCompleted;
            
            franceTitle.classList.add('title-hover');
            // 클리어되지 않은 경우에만 회전 시작
            if (!isCleared) {
                setTimeout(() => {
                    franceTitle.classList.add('title-rotate');
                }, 100);
            }
        }
    });
    franceMapBtn.addEventListener('mouseleave', () => {
        if (franceTitle) {
            franceTitle.classList.remove('title-hover', 'title-rotate');
        }
    });
...
if (backToTitle) {
    backToTitle.addEventListener('click', () => {
        ...
        if (mapScreen && mapScreen.classList.contains('active')) {
            ...
            if (headerSquare) {
                headerSquare.classList.remove('slide-in');
                headerSquare.classList.add('slide-out');
            }
            // 스테이지 버튼들 사라지는 애니메이션
            countryButtons.forEach((btn, index) => {
                setTimeout(() => {
                    btn.classList.remove('map-show');
                }, index * 50); // 각 버튼을 순차적으로 사라지게
            });
            // 국가 타이틀 이미지들 사라지는 애니메이션
            countryTitles.forEach((title, index) => {
                setTimeout(() => {
                    title.classList.remove('map-show');
                }, index * 50); // 각 타이틀을 순차적으로 사라지게
            });
...
const speechSequenceFrance = [
    { text: 'Bonjour! 비스트로 파리에 오신 것을 환영합니다!', buttonText: '안녕하세요' },
    { text: '저희 식당을 찾아 주셔서 감사합니다. 첫 방문이신가요?', buttonText: '네' },
    { text: '걱정 마세요! 쉽진 않지만, 제가 손님께 도움을 드리겠습니다.', buttonText: '고마워요' },
    { text: '첫 번째 순서입니다, 냅킨을 펼쳐볼까요?', buttonText: null, showNextButton: false, onShow: highlightNapkinAndDropZone },
    { text: '처음 순서는 아페리티프(Apéritif) 입니다.', buttonText: '다음' },
    { text: '식전에 빵과 함께 식욕을 돋구는 단계죠.', buttonText: '오호' },
    { text: '빵 좀 드셔보시겠어요?', buttonText: null, showNextButton: false, onShow: highlightRbread },
];
...
function initializeFranceStage() {
    ...
    // 프랑스 스테이지 슬롯 아이템 비활성화 (접시를 놓아볼까요 단계 이전)
    const slotMenuFrance = document.getElementById('slot-menu-france');
    if (slotMenuFrance) {
    ...
    // 프랑스 스테이지에 배치된 냅킨 드래그 활성화
    const franceNapkin = document.getElementById('france-napkin-draggable');
    if (franceNapkin) {
        franceNapkin.setAttribute('draggable', 'true');
        franceNapkin.style.cursor = 'grab';
        franceNapkin.addEventListener('dragstart', handleDragStart);
        franceNapkin.addEventListener('dragend', handleDragEnd);
    }
    
    // 프랑스 스테이지에 배치된 라운드 빵 드래그 활성화
    const franceRbread = document.getElementById('france-rbread-draggable');
    if (franceRbread) {
        franceRbread.setAttribute('draggable', 'true');
        franceRbread.style.cursor = 'grab';
...
function handleDragStart(e) {
    ...
    } else if (this.id === 'france-napkin-draggable' || this.classList.contains('france-napkin')) {
        // 스테이지에 배치된 냅킨인 경우
        e.dataTransfer.setData('source', 'stage');
        
        // 냅킨의 실제 이미지 크기에 맞춰 드래그 이미지 설정
    ...
    } else if (this.id === 'france-rbread-draggable' || this.classList.contains('france-rbread')) {
        // 스테이지에 배치된 라운드 빵인 경우
        e.dataTransfer.setData('source', 'stage');
        
        // 라운드 빵의 실제 이미지 크기에 맞춰 드래그 이미지 설정
        const img = this.querySelector('img');
        if (img) {
            const createDragImage = (naturalWidth, naturalHeight) => {
                // 원본 크기를 vw로 변환
...
function handleDragOver(e) {
    // 비활성화된 드랍존에서는 드래그 오버 막기
    if (this.style.display === 'none' || this.style.pointerEvents === 'none' || this.classList.contains('disabled')) {
        return false;
    }
    if (e.preventDefault) {
        e.preventDefault();
    ...
    if (this.id === 'drop-rbread-utensils' && draggedElement) {
        ...
        const isKnife = draggedElement.classList.contains('france-knife1') || 
                        draggedElement.classList.contains('france-knife2');
        
        if (isHand || isFork || isKnife) {
            const dropZoneText = this.querySelector('.drop-zone-text');
            if (dropZoneText) {
...
function handleDragLeave(e) {
    ...
    // 프랑스 스테이지 빵 드랍존에서 드래그 리브 시 텍스트 복원
    if (this.id === 'drop-rbread-utensils') {
        const dropZoneText = this.querySelector('.drop-zone-text');
        if (dropZoneText) {
...
function handleDrop(e) {
    ...
    if (source === 'dropped') {
        ...
        // 슬롯, 셰프테이블, 또는 스테이지에 배치된 요소에서 드래그한 경우
        if (!draggedElement) return;

        itemType = draggedElement.getAttribute('data-item');
        imageSrc = draggedElement.getAttribute('data-image');
        
        // 스테이지에 배치된 요소인 경우 이미지 소스 직접 가져오기 및 itemType 설정
        if (source === 'stage' && !imageSrc) {
            ...
            if (draggedElement.classList.contains('france-fork0')) {
                ...
                itemType = 'fork1';
            } else if (draggedElement.classList.contains('france-fork2')) {
                itemType = 'fork2';
            } else if (draggedElement.classList.contains('france-knife1')) {
                itemType = 'knife1';
            } else if (draggedElement.classList.contains('france-knife2')) {
                itemType = 'knife2';
    ...
    // 일본 스테이지에서 수저를 밥그릇 꽂기/집기 드랍존이 아닌 다른 곳에 드롭하면 아무 반응 없음
    const stageCheck = document.getElementById('japan-stage')?.classList.contains('active') ? 'japan' :
    ...
    if (stageCheck === 'japan' && (itemType === 'spoon' || itemType === 'chopsticks')) {
        // 밥그릇 관련 드랍존이 아닌 경우 아무 반응 없음
        const isRiceBowlRelatedZone = dropZoneId === 'rice-bowl-drop-zone' || 
                                     this.closest('#rice-bowl-drop-zone') !== null ||
                                     this.closest('.rice-bowl-zone-left') !== null ||
                                     this.closest('.rice-bowl-zone-right') !== null ||
                                     dropZoneId === 'grab-drop-zone' ||
                                     this.closest('#grab-drop-zone') !== null ||
                                     this.closest('.grab-zone-left') !== null ||
                                     this.closest('.grab-zone-right') !== null ||
                                     dropZoneId === 'drop-spoon' ||
                                     dropZoneId === 'drop-chopsticks' ||
                                     dropZoneId === 'drop-chopsticks-2';
        
        if (!isRiceBowlRelatedZone) {
            // 밥그릇 관련 드랍존이 아니면 아무 반응 없음
            return false;
        }
    }
    ...
    // 프랑스 스테이지 빵 드랍존에 식기나 손 드롭 방지
    if (currentStage === 'france' && dropZoneId === 'drop-rbread-utensils' && 
    ...
    if (source === 'stage' && draggedElement) {
        ...
        const rbreadUtensilsDropZone = document.getElementById('drop-rbread-utensils');
        
        const allItems = [fork0, fork1, fork2, knife1, knife2, hand];
        ...
        if (rbreadUtensilsDropZone) {
            rbreadUtensilsDropZone.classList.remove('highlight-pulse');
            // 드랍존 숨기기
            rbreadUtensilsDropZone.style.display = 'none';
        }
        
        // 빵 드랍존에 배치된 빵 이미지를 fr_rbread_a.png로 교체 (애니메이션 포함)
        const dropRbreadZone = document.getElementById('drop-rbread');
        if (dropRbreadZone) {
            ...
            if (droppedRbread) {
                ...
                if (mainImg) {
                    ...
                    if (mainImg.complete) {
                        ...
                        mainImg.onload = animateImage;
                    }
                }
            }
        }
        
        // 말풍선 및 빵 먹는 예절 인포 메뉴 표시
        setTimeout(() => {
            showSpeechBubbleFrance('좋습니다!', 3000, false, null, null, null, null);
            showBreadEtiquetteInfoMenuFrance();
        }, 250);
    ...
    if (currentStage === 'france' && itemType === 'napkin' && dropZoneId === 'drop-napkin') {
        ...
        // 하이라이팅 제거
        const napkinElement = document.getElementById('france-napkin-draggable');
        const napkinDropZone = document.getElementById('drop-napkin');
        if (napkinElement) {
            napkinElement.classList.remove('highlight-pulse');
        }
        if (napkinDropZone) {
    ...
    if (currentStage === 'france' && itemType === 'rbread') {
        ...
        const dish2DropZone = document.getElementById('drop-dish2');
        if (rbreadElement) {
            rbreadElement.classList.remove('highlight-pulse');
        }
        if (rbreadDropZone) {
            rbreadDropZone.classList.remove('highlight-pulse');
        }
        if (dish2DropZone) {
            dish2DropZone.classList.remove('highlight-pulse');
        }
        
        if (dropZoneId === 'drop-dish2') {
            // dish2에 드롭된 경우 말풍선 및 인포 메뉴 표시
            setTimeout(() => {
                showSpeechBubbleFrance('거기가 아닐텐데요', 3000, false, null, null, null, null);
                showBreadPlacementInfoMenuFrance();
            }, 250);
        } else if (dropZoneId === 'drop-rbread') {
            // 테이블 쪽에 드롭된 경우 드랍존 비활성화 및 말풍선 표시
            // 드랍존 비활성화
            if (rbreadDropZone) {
                rbreadDropZone.style.pointerEvents = 'none';
                rbreadDropZone.classList.add('disabled');
            }
            if (dish2DropZone) {
                dish2DropZone.style.pointerEvents = 'none';
                dish2DropZone.style.display = 'none';
                dish2DropZone.classList.add('disabled');
            }
            // 말풍선 표시 및 포크/나이프 활성화
            setTimeout(() => {
                enableForksAndKnives();
                showSpeechBubbleFrance('빵을 드시는 법은 아실테지요', 0, false, null, null, null, null);
                showBreadPlacementInfoMenuFrance();
            }, 250);
        }
    }
    ...
    if (currentStage === 'france' && itemType === 'dish1' && dropZoneId === 'drop-dish1') {
        ...
        // 말풍선 표시
        setTimeout(() => {
            showSpeechBubbleFrance('좋습니다!', 2000, false, null, null, null, null);
        }, 250);
        
        // 3초 뒤에 드랍존에 드롭된 dish1 이미지를 fr_dish1_a.png로 변경 (fade-out/in 동시 애니메이션)
        setTimeout(() => {
            const droppedDish1 = dish1DropZone.querySelector('.dropped-item');
            if (droppedDish1) {
                // 메인 이미지 찾기 (shadow-img가 아닌 이미지)
                ...
                if (mainImg) {
                    ...
                    // 이미지 미리 로드
                    const newImg = new Image();
                    newImg.src = 'resource/fr/fr_dish1_a.png';
                    ...
                    const changeImage = () => {
                        ...
                        // fade-out이 절반 진행될 때 이미지 교체 (동시에 진행)
                        setTimeout(() => {
                            mainImg.src = 'resource/fr/fr_dish1_a.png';
...
function placeItemInZone(itemType, imageSrc, dropZone, dropZoneId, correctZoneId, slotElement) {
    ...
    // 이미지 크기 설정 함수 (기존 이미지 사이즈 유지) - vw 단위
    const setImageSize = function () {
        const naturalWidth = img.naturalWidth;
        const naturalHeight = img.naturalHeight;

        // 원본 크기 그대로 유지
        img.style.width = `${pxToVw(naturalWidth)}vw`;
        img.style.height = `${pxToVw(naturalHeight)}vw`;
    };
    ...
    if (isCorrect) {
        ...
        // 일본 스테이지에서 수저 드랍 시 cutlery.mp3 재생
        if (currentStage === 'japan' && (itemType === 'spoon' || itemType === 'chopsticks')) {
            try {
                const cutleryAudio = new Audio('resource/sound/cutlery.mp3');
                cutleryAudio.volume = 0.5;
                cutleryAudio.currentTime = 0;
        ...
        // 일본 스테이지에서 밥그릇 드랍 시 dish.mp3 재생
        if (currentStage === 'japan' && itemType === 'rice-bowl') {
            try {
                const dishAudio = new Audio('resource/sound/dish.mp3');
                dishAudio.volume = 0.5;
                dishAudio.currentTime = 0;
        ...
        if (itemType === 'rice-bowl') {
            ...
            if (itemType === 'spoon' || itemType === 'chopsticks') {
                if (currentStage === 'japan') {
                    stopHighlightUtensils();
                    // 젓가락 놓기 예절 인포 메뉴 표시 (젓가락만)
                    if (itemType === 'chopsticks') {
                        showUtensilPlacementInfoMenu();
                    }
                } else {
            ...
            // 젓가락이 올바르게 배치된 경우 젓가락 놓기 예절 인포 메뉴 표시
            if (itemType === 'chopsticks') {
                if (currentStage === 'japan') {
                    showUtensilPlacementInfoMenu();
                } else {
...
// 젓가락 놓기 예절 정보 메뉴 닫기
function closeUtensilPlacementInfoMenu() {
    const infoMenu = document.getElementById('utensil-placement-info-menu');
    if (!infoMenu) return;
...
function showSpoonForbiddenInfoMenu() {
    ...
    // 이미 표시되어 있는지 확인 (computed style로 확인)
    const computedRight = window.getComputedStyle(infoMenu).right;
    const isAlreadyVisible = infoMenu.style.display === 'block' && 
                             (computedRight === '20px' || infoMenu.style.right === '20px');

    if (isAlreadyVisible) {
        // 이미 표시되어 있으면 다시 나타나는 애니메이션
        infoMenu.style.transition = 'none';
        infoMenu.style.right = '-25vw';
        // 리플로우 강제
...
// 숟가락 사용 금지 정보 메뉴 닫기
function closeSpoonForbiddenInfoMenu() {
    const infoMenu = document.getElementById('spoon-forbidden-info-menu');
    if (!infoMenu) return;
...
function replaceWithGrabImage() {
    ...
    if (spoonDropZone) {
        spoonDropZone.style.pointerEvents = 'none';
    }
    if (chopsticksDropZone) {
        chopsticksDropZone.style.pointerEvents = 'none';
    }
    if (chopsticksDropZone2) {
        chopsticksDropZone2.style.pointerEvents = 'none';
    }
    ...
    if (riceBowlRect) {
        ...
        const topVw = pxToVw(riceBowlRect.top - tableSettingRect.top + riceBowlRect.height / 2 + 50);
        ...
        grabImageContainer.style.top = '60%';
        grabImageContainer.style.transform = 'translate(-50%, -50%)';
    }
...
function showNextSpeechFrance() {
    ...
    if (currentSpeechIndexFrance < speechSequenceFrance.length) {
        ...
        const buttonText = typeof currentSpeech === 'string' ? BUTTON_TEXTS.nextFrance : (currentSpeech.buttonText !== undefined ? currentSpeech.buttonText : BUTTON_TEXTS.nextFrance);
        const buttons = typeof currentSpeech === 'string' ? null : (currentSpeech.buttons || null);
        const onShow = typeof currentSpeech === 'string' ? null : (currentSpeech.onShow || null);
        const showNextButton = typeof currentSpeech === 'string' ? !isLast : (currentSpeech.showNextButton !== undefined ? currentSpeech.showNextButton : !isLast);

        console.log('대사 정보:', { currentText, buttonText, isLast, showNextButton });
        ...
        showSpeechBubbleFrance(currentText, 0, showNextButton, nextCallback, onShow, buttonText, buttons);
    } else {
...
// 냅킨과 냅킨 드랍존 하이라이팅
function highlightNapkinAndDropZone() {
    const napkinElement = document.getElementById('france-napkin-draggable');
    const napkinDropZone = document.getElementById('drop-napkin');
    
    if (napkinElement) {
        napkinElement.classList.add('highlight-pulse');
    }
    if (napkinDropZone) {
        napkinDropZone.classList.add('highlight-pulse');
    }
}

// 라운드 빵 하이라이팅
function highlightRbread() {
    const rbreadElement = document.getElementById('france-rbread-draggable');
    const rbreadDropZone = document.getElementById('drop-rbread');
    const dish2DropZone = document.getElementById('drop-dish2');
    
    if (rbreadElement) {
        rbreadElement.classList.add('highlight-pulse');
    }
    if (rbreadDropZone) {
        rbreadDropZone.style.display = 'flex';
        rbreadDropZone.classList.add('highlight-pulse');
    }
    if (dish2DropZone) {
        dish2DropZone.style.display = 'flex';
        dish2DropZone.classList.add('highlight-pulse');
    }
}
...
function enableForksAndKnives() {
    ...
    if (hand) {
        ...
        hand.classList.add('highlight-pulse');
        hand.addEventListener('dragstart', handleDragStart);
        hand.addEventListener('dragend', handleDragEnd);
    }
    
    // 빵 위 식기 드랍존 표시 및 하이라이팅
    const rbreadUtensilsDropZone = document.getElementById('drop-rbread-utensils');
    if (rbreadUtensilsDropZone) {
        rbreadUtensilsDropZone.style.display = 'flex';
        rbreadUtensilsDropZone.classList.add('highlight-pulse');
    }
}
...
function showSpeechBubbleFrance(text, duration = 3000, showNextButton = false, nextCallback = null, onStartCallback = null, buttonText = null, buttons = null) {
    ...
    currentTypingInterval = setInterval(() => {
        if (index < text.length) {
            ...
            // 타이핑 완료 후 다음 버튼 표시 (showNextButton이 true이거나 buttonText가 있는 경우, 단 buttonText가 null이면 표시 안 함)
...
// 프랑스 스테이지 빵 먹기 예절 정보 메뉴 닫기
function closeBreadEtiquetteInfoMenuFrance() {
    const infoMenu = document.getElementById('bread-etiquette-info-menu-france');
    if (!infoMenu) return;
    infoMenu.style.transition = 'right 0.5s ease-in';
    infoMenu.style.right = '-25vw';
...
function handleDishSelection(choice) {
    if (choice === 'leave') {
        ...
        // 새로운 말풍선 표시: "...입에 안 맞으셨나요?"
        // ◀ 버튼 클릭 시 이전 말풍선으로 돌아가기
        const onNextClick = () => {
            expandSpeechBubbleForDishSelection();
        };
        
        showSpeechBubble('...입에 안 맞으셨나요?', -1, true, onNextClick, null, '◀', null);
...
function handleAfterMealSpeechSelection(choice) {
    if (choice === 'correct') {
        ...
        // 오답 처리 (이타다키마스 또는 오이시카타데스)
        // 기존 말풍선을 기본 스타일로 되돌리기
        resetSpeechBubbleToDefault();

        // 새로운 말풍선 표시: "아닙니다. 다시 생각해보세요."
        // ◀ 버튼 클릭 시 이전 말풍선으로 돌아가기
        const onNextClick = () => {
            // 말풍선을 기본 스타일로 되돌리기
            resetSpeechBubbleToDefault();
            // "이런 때 보통, 해야 할 말이 있죠?" 말풍선 다시 표시
            showAfterMealSpeechSelection();
        };
        
        showSpeechBubble('아닙니다. 다시 생각해보세요.', -1, true, onNextClick, null, '◀', null);
...
function showBillFromCheftable() {
    ...
    const setupPositionAndAnimate = () => {
        ...
        const naturalWidth = img.naturalWidth;
        const naturalHeight = img.naturalHeight;
        const scaledWidth = naturalWidth * 0.6; // 크기 증가
        const scaledHeight = naturalHeight * 0.6;
        img.style.width = `${pxToVw(scaledWidth)}vw`;
        img.style.height = `${pxToVw(scaledHeight)}vw`;
        ...
        // CSS에서 left, transform은 이미 설정되어 있으므로 top과 opacity만 설정
...
function showBillDetail() {
    ...
    billDetail.style.left = '45%'; // 왼쪽으로 이동
    ...
    const closeHandler = (e) => {
        ...
        // 결제 선택 버튼을 클릭했을 때는 닫히지 않도록 예외 처리
        // 말풍선 내부의 버튼(카드/현금 버튼)을 클릭했을 때
        const clickedButton = e.target.closest('button');
        if (clickedButton && clickedButton.textContent && 
            (clickedButton.textContent.includes('카드로 계산') || clickedButton.textContent.includes('현금으로 계산'))) {
            return;
        }
...
function handlePaymentSelection(choice) {
    if (choice === 'card') {
        // 카드로 계산 시: 기존 말풍선을 기본 스타일로 되돌리기
        resetSpeechBubbleToDefault();

        // 새로운 말풍선 표시: "죄송하지만 저희는 현금만 받고 있어요"
        // ◀ 버튼 클릭 시 이전 말풍선으로 돌아가기
        const onNextClick = () => {
            // 말풍선을 기본 스타일로 되돌리기
            resetSpeechBubbleToDefault();
            // "결제는 어떻게 하시겠습니까?" 말풍선 다시 표시
            expandSpeechBubbleForPaymentSelection();
        };
        
        showSpeechBubble('죄송하지만 저희는 현금만 받고 있어요.', -1, true, onNextClick, null, '◀', null);
...
function showStageClearEffect(stage) {
    ...
    if (stage === 'japan') {
        ...
        // 지도 화면의 일본 타이틀 이미지를 클리어 버전으로 변경 (현재 페이지에 있는 경우)
        const japanTitleImg = document.querySelector('.japan-title');
        if (japanTitleImg) {
            japanTitleImg.src = 'resource/title/TitleJp_F.png';
        }
    } else if (stage === 'china') {
        ...
        // 지도 화면의 중국 타이틀 이미지를 클리어 버전으로 변경 (현재 페이지에 있는 경우)
        const chinaTitleImg = document.querySelector('.china-title');
        if (chinaTitleImg) {
            chinaTitleImg.src = 'resource/title/TitleCn_F.png';
        }
    ...
    // 클리어 사운드 재생
    try {
        const clearAudio = new Audio('resource/sound/247. Receiving Money.mp3');
        clearAudio.volume = 0.5;
        clearAudio.currentTime = 0;
        clearAudio.preload = 'auto';
        clearAudio.play().catch(err => {
            console.log('클리어 사운드 재생 실패:', err);
        });
    } catch (err) {
        console.log('클리어 사운드 재생 오류:', err);
    }
...
function clearDishesAndComplete() {
    ...
    selectorsToHide.forEach(selector => {
        ...
        elements.forEach(el => {
            ...
            // 상단으로 이동하며 사라짐
            // 모든 요소가 현재 위치에서 위로 이동하도록 처리
            // 그림자 요소들은 transform으로 위치가 잡혀있으므로, translateY를 추가하여 이동
            if (currentTransform && currentTransform !== 'none') {
                // 이미 transform이 있는 경우 (그림자 요소 등)
                el.style.transform = `${currentTransform} translateY(-150px)`;
            } else {
                // transform이 없는 경우
